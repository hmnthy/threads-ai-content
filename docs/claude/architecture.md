...
| `/` đổi thành landing/story page (Tầng B scoped: Hero+Problem+Solution+Tech stack+Coming soon+Author, KHÔNG phải bản đầy đủ 13-section của design-system.md §7), tool (Overview/Analytics/Topic Explorer) chuyển sang `/overview`+`/analytics`+`/topics` qua Next.js route group `(app)/` | User test thật link deploy tạm, chỉ ra ai bấm link cũng rơi thẳng vào raw analytics, không có gì giải thích sản phẩm — đúng vấn đề mà design-system.md §7 đã dự đoán từ trước khi tách Tầng A/B ("Tầng B = trang public giới thiệu tool"), chỉ là đã hoãn tới giờ mới làm, và chỉ làm đúng phần được yêu cầu chứ không phải toàn bộ 13-section — quyết định 2026-09-03 |
| Tech stack hiển thị công khai trên landing page dùng đúng trạng thái live/coming soon THẬT của code hiện tại (không phải kỳ vọng roadmap) | Vector store/RAG, fixed-category classifier (SVM-RBF/LogReg), và carousel generation (Pillow) đều đánh dấu "coming soon" vì code thật sự chưa tồn tại (`src/generation/` không có, `src/carousel/` chỉ có font) — tránh hứa hẹn tính năng cho người xem portfolio mà chưa có, đúng nguyên tắc evidence-based đã lập từ đầu dự án — quyết định 2026-09-03 |
